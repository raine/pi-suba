...
async function runFake(task: string, input?: string): Promise<{ artifactDir: string; code: number }> {
  ...
  const child = spawn(resolve("test/fixtures/fake-pi"), ["--session", sessionFile, "--model", "fake/model", task], {
    env: { ...process.env, SUBA_CHILD_ID: "fixture", SUBA_ARTIFACT_DIR: artifactDir, SUBA_SESSION_FILE: sessionFile, SUBA_AUTO_COMPLETE: "1" },
    stdio: ["pipe", "ignore", "pipe"],
  });
  if (input !== undefined) child.stdin.end(input); else child.stdin.end();
  const code = await new Promise<number>((resolveCode, reject) => { child.on("error", reject); child.on("close", (value) => resolveCode(value ?? -1)); });
...
describe("fake Pi fixture", () => {
  it("records launch arguments and completes", async () => {
    ...
    expect((await extractResult(join(artifactDir, "session.jsonl"))).text).toContain("inspect files");
  ...
  it("waits for parent guidance", async () => {
    ...
    expect((await readEvents(artifactDir, 0)).map((event) => event.type)).toEqual(["ping", "completed"]);
    expect(await readFile(join(artifactDir, "received-guidance.txt"), "utf8")).toContain("second line");
  });
