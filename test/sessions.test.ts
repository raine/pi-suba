...
describe("sessions", () => {
  it("seeds lineage and excludes the triggering user turn from forks", async () => {
    const entries = [
      ...
      assistant("answer"),
      { type: "message", id: "b", message: { role: "user", content: "spawn" } },
      assistant("tool"),
    ]
    expect(forkEntries(entries)).toEqual(entries.slice(0, 2))
    const dir = await mkdtemp(join(tmpdir(), "suba-session-"))
  ...
  it("extracts only appended results and provider errors", async () => {
    const dir = await mkdtemp(join(tmpdir(), "suba-result-"))
    const path = join(dir, "session.jsonl")
