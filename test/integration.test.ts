...
async function waitForCompletion(artifactDir: string): Promise<void> {
  ...
  throw new Error(`Pi child did not complete within ${timeout}ms. Check model authentication and SUBA_TEST_MODEL.`);
}

describe.skipIf(!model)("real Pi integration", () => {
  afterAll(async () => { try { await exec("tmux", ["-L", socket, "kill-server"]); } catch {} });
  it("runs a persistent interactive child through automatic completion", async () => {
    await exec("tmux", ["-V"]);
    ...
    expect((await extractResult(sessionFile)).text?.toLowerCase()).toContain("pi-suba integration ok");
    ...
    expect((await readFile(sessionFile, "utf8")).split("\n").filter(Boolean).length).toBeGreaterThan(1);
