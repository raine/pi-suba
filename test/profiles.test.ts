...
describe("profiles", () => {
  it("parses frontmatter and body", () => {
    ...
    expect(profile).toMatchObject({ name: "explore", tools: "read-only", loadContext: false, loadSkills: false, systemPrompt: "replace", autoComplete: false, thinking: "low", body: "Explore.\n" });
  ...
  it("rejects arbitrary tools and keys", () => {
    expect(() => parseProfile("---\ntools: read,write\n---\n", "x")).toThrow("unsupported tools policy");
