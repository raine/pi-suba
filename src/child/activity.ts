...
export class ActivityRecorder {
  ...
  async update(
    patch: Partial<Omit<ChildActivity, "version" | "childId" | "sequence" | "updatedAt">>,
  ): Promise<void> {
    this.sequence += 1
    this.snapshot = {
      ...this.snapshot,
  ...
  streaming(): Promise<void> {
    return this.update({
      ...
      toolName: undefined,
      activityStartedAt:
        this.snapshot.activity === "streaming" ? this.snapshot.activityStartedAt : Date.now(),
    })
  }
  ...
  selected(provider: string, model: string): Promise<void> {
    return this.update({ provider, model })
  }
  thinking(thinking: ThinkingLevel): Promise<void> {
    return this.update({ thinking })
  }
}
