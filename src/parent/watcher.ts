...
export class ChildWatcher {
  ...
  constructor(private readonly pollMs: number, private readonly callbacks: WatcherCallbacks) {}
  start(): void { if (!this.timer) this.timer = setInterval(() => void this.tick(), this.pollMs); }
  ...
  async check(id: string): Promise<void> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
        record = { ...record, activityError: error instanceof Error ? error.message : String(error) };
        this.callbacks.activity(record);
      }
    }
    let events: ChildEvent[];
    try { events = await readEvents(record.artifactDir, record.lastEventSequence); }
    catch (error) {
      this.callbacks.activity({ ...record, activityError: error instanceof Error ? error.message : String(error) });
      return;
    }
    for (const event of events) {
      if (event.childId !== id || event.sequence !== record.lastEventSequence + 1) {
        this.callbacks.activity({ ...record, activityError: `invalid event sequence ${event.sequence}` });
        return;
      }
      record = { ...record, revision: record.revision + 1, lastEventSequence: event.sequence };
      if (event.type === "ping") {
        record.state = "awaiting-parent";
        this.callbacks.update(record, { type: "ping", content: `Subagent ${record.name} (${id}) needs guidance:\n\n${event.message}` });
      } else if (event.type === "failed") {
        record.state = "failed"; record.error = event.error;
        this.callbacks.update(record, { type: "result", content: `Subagent ${record.name} (${id}) failed: ${event.error}` });
        return;
      } else {
    ...
    try {
      ...
      const next = { ...record, revision: record.revision + 1, state: failed ? "failed" as const : "completed" as const, result: result.text, error: result.error ?? (exitCode ? `Pi exited with status ${exitCode}` : undefined) };
      this.callbacks.update(next, { type: "result", content: failed ? `Subagent ${record.name} (${id}) failed: ${next.error}` : `Subagent ${record.name} (${id}) completed:\n\n${next.result ?? "(no assistant text)"}` });
      return;
    } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    if (record.paneId && !(await paneExists(record.paneId))) {
      const next = { ...record, revision: record.revision + 1, state: "interrupted" as const, error: "tmux pane closed before completion" };
      this.callbacks.update(next, { type: "result", content: `Subagent ${record.name} (${id}) was interrupted because its tmux pane closed.` });
    }
  }
}
