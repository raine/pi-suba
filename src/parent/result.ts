...
export function resultBodyLines(content: string): string[] {
  const lines = resultBody(content)
    .split(/\r?\n/)
...
export function collapsedCompletedResult(
  name: string,
  id: string,
  content: string,
...
export function renderSubagentMessage(
  ...
  if (invocation.length)
    container.addChild(new Text(theme.fg("dim", `  ${invocation.join(" · ")}`), 0, 0))

  const body =
    resultBody(content) ||
    details.error ||
    (details.state === "completed" ? "(no assistant text)" : content)
  if (expanded) {
    ...
    if (preview.lines.length) {
      container.addChild(new Spacer(1))
      container.addChild(
        new Text(preview.lines.map((line) => theme.fg("toolOutput", line)).join("\n"), 0, 0),
      )
    }
    if (preview.hidden)
      container.addChild(
        new Text(
          theme.fg("muted", `… ${preview.hidden} more ${preview.hidden === 1 ? "line" : "lines"}`),
          0,
          0,
        ),
      )
