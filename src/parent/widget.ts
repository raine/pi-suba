...
export function projectWidget(
  ...
  const active = records.filter((record) =>
    ["launching", "running", "awaiting-parent"].includes(record.state),
  )
  const items = active.slice(0, Math.max(0, maxRows)).map(
    (record): WidgetItem => ({
      id: record.id,
      name: record.name,
  ...
  return {
    ...
    items,
    overflow: Math.max(0, active.length - items.length),
  }
}
...
export class ActivityWidget implements Component {
  ...
  render(width: number): string[] {
    ...
    if (width < 4)
      return [truncateToWidth(this.theme.fg("accent", `${projection.activeCount}●`), width)]
    ...
    if (projection.overflow) {
      ...
      lines.push(
        `${this.theme.fg("accent", "│")}${body}${" ".repeat(Math.max(0, innerWidth - visibleWidth(body)))}${this.theme.fg("accent", "│")}`,
      )
    }
