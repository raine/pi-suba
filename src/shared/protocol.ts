...
export type ChildEvent =
  | { version: 1; childId: string; sequence: number; timestamp: number; type: "ping"; message: string }
  | { version: 1; childId: string; sequence: number; timestamp: number; type: "completed" }
  | { version: 1; childId: string; sequence: number; timestamp: number; type: "failed"; error: string };
...
export async function readEvents(artifactDir: string, afterSequence: number): Promise<ChildEvent[]> {
  ...
  for (const name of names.filter((item) => /^\d{6}-(ping|completed|failed)\.json$/.test(item)).sort()) {
    ...
    if (event.version !== 1 || event.sequence !== sequence) throw new Error(`invalid event file: ${name}`);
    result.push(event);
  }
  return result;
}
