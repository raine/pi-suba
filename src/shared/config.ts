...
export function parsePlacement(value: unknown, label = "placement"): Placement {
  ...
  for (const key of Object.keys(input)) if (key !== "type" && key !== "windowName") throw new Error(`${label} has unsupported key: ${key}`);
  if (input.type !== "split" && input.type !== "window" && input.type !== "shared-window") throw new Error(`${label}.type is invalid`);
  if (input.type === "split" && input.windowName !== undefined) throw new Error(`${label}.windowName is invalid for split placement`);
  const windowName = input.windowName === undefined ? undefined : string(input.windowName, `${label}.windowName`);
...
export function parseConfig(value: unknown): SubaConfig {
  ...
  for (const key of Object.keys(activityInput)) if (!["pollMs", "staleAfterMs", "maxRows"].includes(key)) throw new Error(`activity has unsupported key: ${key}`);
  const thinking = input.thinking === undefined ? undefined : string(input.thinking, "thinking") as ThinkingLevel;
  if (thinking && !THINKING_LEVELS.includes(thinking)) throw new Error(`thinking is invalid: ${thinking}`);
  return {
    defaultProfile: input.defaultProfile === undefined ? DEFAULT_CONFIG.defaultProfile : string(input.defaultProfile, "defaultProfile"),
    ...
    activity: {
      ...
      maxRows: activityInput.maxRows === undefined ? DEFAULT_CONFIG.activity.maxRows : integer(activityInput.maxRows, "activity.maxRows", 1, 100),
    },
    sharedWindowName: input.sharedWindowName === undefined ? DEFAULT_CONFIG.sharedWindowName : string(input.sharedWindowName, "sharedWindowName"),
  };
}
